...
fn main() -> io::Result<()> {
    ...
    if video_files.len() < 2 {
        println!("Not enough video files to merge.");
        return Ok(());
    }

    // Take the last two video files
    let last_index = video_files.len() - 1;
    let second_last_index = last_index - 1;

    let file1 = &video_files[second_last_index];
    let file2 = &video_files[last_index];

    println!("Combining {} and {}", file1, file2);

    // Check if re-encoding is required for the last two video files
    let reencoded_file1 = format!("reencoded_{}.mp4", second_last_index);
    let reencoded_file2 = format!("reencoded_{}.mp4", last_index);
    ...
    let output_file = format!("combined_{}.mp4", second_last_index);
    ...
    match status {
        Ok(status) if status.success() => {
            ...
            // Replace the second last file with the combined output
